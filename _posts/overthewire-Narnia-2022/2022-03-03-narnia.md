...
The code is inconsistent, at least for me, doing the same thing  managed to only get me a handful of correct results. From what I could tell, doing these set of shell commands from login is the most reliable way to get access to the shell. My guess is either the shellcode that was running  wrong, or that the c code to find the address of the environment variable was somehow wrong. I pulled the shellcode and general idea from [this](https://www.tenouk.com/Bufferoverflowc/bufferoverflowvulexploitdemo32.html) tutorial, this also gave the code for the addr.c code.

One thing that we can do, since we don't know the memory location in which to return to, a place where the memory location can we can find and stay consistent is stored as an environment variable. Saving the shellcode in `SHELLCODE`, similar to how we did it in `Level 1`. The shellcode that we used for `Level 1` did not work as expected, so I used the alternate shellcode that I also mentioned.
